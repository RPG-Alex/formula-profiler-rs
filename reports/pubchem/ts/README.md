...
This report treats each spectrum as **positive** when its molecular formula contains the target element `Ts`. A spectrum is **negative** when its formula does not contain `Ts`.
...
This section shows how many formula-bearing spectra have exactly `k` atoms of `Ts`.
